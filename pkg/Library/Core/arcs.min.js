...
`).trim();e?.message?t.error(e.message,...r,`(${a})`):t(e,...r,`(${a})`)}};f.particleIndustry=$e;f.securityLockdown=ye;var Z={};st(Z,{PathMapper:()=>$t,Paths:()=>d,arand:()=>M,async:()=>Oe,asyncTask:()=>je,computeAgeString:()=>we,debounce:()=>Pe,deepCopy:()=>m,deepEqual:()=>u,deepUndefinedToNull:()=>ct,irand:()=>O,key:()=>Mt,logFactory:()=>o,makeId:()=>E,matches:()=>J,prob:()=>Nt,shallowMerge:()=>Ut,shallowUpdate:()=>zt});var we=(s,t)=>{let e=Math.round((t-s)/1e3);if(isNaN(e))return"\u2022";let r="";return e<60?(e>1&&(r="s"),`${e} second${r} ago`):(e=Math.round(e/60),e<60?(e>1&&(r="s"),`${e} minute${r} ago`):(e=Math.round(e/60),e<24?(e>1&&(r="s"),`${e} hour${r} ago`):(e=Math.round(e/24),e<30?(e>1&&(r="s"),`${e} day${r} ago`):(e=Math.round(e/30),e<12?(e>1&&(r="s"),`${e} month${r} ago`):(e=Math.round(e/12),e>1&&(r="s"),`${e} year${r} ago`)))))};var Pe=(s,t,e)=>{if(s&&clearTimeout(s),t&&e)return setTimeout(t,e)},Oe=s=>async(...t)=>{await Promise.resolve(),s(...t)},je=(s,t)=>{setTimeout(s,t??0)};var{logFactory:Is,Paths:Ee}=Z;var Ce=import.meta.url.split("/").slice(0,-1).join("/");Ee.setRoot(Ce);export{P as Arc,yt as Chef,H as DataStore,I as Decorator,p as EventEmitter,$ as Host,v as Parser,x as ParticleCook,Ee as Paths,f as Runtime,j as Store,C as StoreCook,ne as fetchParticleCode,ye as initVanilla,Is as logFactory,ce as maybeFetchParticleCode,G as pathForKind,F as requireParticleBaseCode,bt as requireParticleImplCode,Z as utils};
/**
 * @license
