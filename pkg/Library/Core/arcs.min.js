...
`).trim();e?.message?t.error(e.message,...r,`(${a})`):t(e,...r,`(${a})`)}};f.particleIndustry=$e;f.securityLockdown=be;var Z={};st(Z,{PathMapper:()=>bt,Paths:()=>d,arand:()=>L,async:()=>ke,asyncTask:()=>De,computeAgeString:()=>Ce,debounce:()=>Fe,deepCopy:()=>m,deepEqual:()=>u,deepUndefinedToNull:()=>ct,irand:()=>j,key:()=>Nt,logFactory:()=>n,makeId:()=>C,matches:()=>J,prob:()=>Rt,shallowMerge:()=>Ut,shallowUpdate:()=>zt});var Ce=(s,t)=>{let e=Math.round((t-s)/1e3);if(isNaN(e))return"\u2022";let r="";return e<60?(e>1&&(r="s"),`${e} second${r} ago`):(e=Math.round(e/60),e<60?(e>1&&(r="s"),`${e} minute${r} ago`):(e=Math.round(e/60),e<24?(e>1&&(r="s"),`${e} hour${r} ago`):(e=Math.round(e/24),e<30?(e>1&&(r="s"),`${e} day${r} ago`):(e=Math.round(e/30),e<12?(e>1&&(r="s"),`${e} month${r} ago`):(e=Math.round(e/12),e>1&&(r="s"),`${e} year${r} ago`)))))};var Fe=(s,t,e)=>{if(s&&clearTimeout(s),t&&e)return setTimeout(t,e)},ke=s=>async(...t)=>{await Promise.resolve(),s(...t)},De=(s,t)=>{setTimeout(s,t??0)};var{logFactory:Ws,Paths:Be}=Z;var Te=import.meta.url.split("/").slice(0,-1).join("/");Be.setRoot(Te);export{O as Arc,yt as Chef,I as DataStore,R as Decorator,p as EventEmitter,b as Host,v as Parser,x as ParticleCook,Be as Paths,f as Runtime,E as Store,F as StoreCook,ce as fetchParticleCode,be as initVanilla,Ws as logFactory,le as maybeFetchParticleCode,G as pathForKind,k as requireParticleBaseCode,$t as requireParticleImplCode,Z as utils};
/**
 * @license
