...
({
  ...
  async update({stream, profile, persona}, state, {service, invalidate}) {
    //log(profile);
    ...
    state.streams = await this.updateStreams(state, {service});
  ...
  async updateStreams({streams, lobby, persona}, {service}) {
    const newStreams = await service({kind: 'LobbyService', msg: 'meetStrangers', data: {lobby, persona}});
    streams = [
      ...(streams || []),
      ...(newStreams || [])
    ];
...
<!--
  <div strangers flex column repeat="stranger_t">{{strangers}}</div>
<template stranger_t>
  <div>{{name}}</div>
</template>
-->
